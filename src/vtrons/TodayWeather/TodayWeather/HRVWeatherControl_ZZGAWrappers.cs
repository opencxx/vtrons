...
namespace HighResolutionApps.VisualControls.TodayWeather
{
    ...
    class HRVUserControlWrappers : IVisualControlDescriptor
    {
        ...
        public Type Type
        {
            get { return typeof(TodayWeather); }
        }
        /// <summary>
        /// 组件操作类型
        /// </summary>
    ...
    public class HRVUserControlPropProxy : HighResolutionApps.Common.Schema.BasePropProxy
    {
        ...
        public override PropertyDescriptorCollection GetProperties(Attribute[] attributes)
        {
            ...
            // 自定义属性
            RegisterProperty(typeof(TodayWeather), "InputDataTable", null, result, "绑定数据。此处无需设置数值，但必须手工在右键绑定数据窗口内，给其创建一个“数据表类型”通道，并绑定需要展示的数据", "数据");
            ...
            RegisterProperty(typeof(TodayWeather), "IsEditableInOperater", null, result, "是否可以任意移动、改变大小、角度改变", "基础");
            RegisterProperty(typeof(TodayWeather), "IsMustInOnlyOneMachine", null, result, "是否可以跨屏，跨渲染节点显示", "基础");
            RegisterProperty(typeof(TodayWeather), "MemoryRequest", null, result, "内存需求", "基础");
            RegisterProperty(typeof(TodayWeather), "ControlInstanceDes", null, result, "组件示例描述", "基础");
            // 位置尺寸
            ...
            // 组别: 外观
            RegisterProperty(typeof(TodayWeather), "ControlInstanceMainTitle", null, result, "窗口默认主标题", "外观");
            RegisterProperty(typeof(TodayWeather), "ControlInstanceSubTitle", null, result, "窗口默认副标题", "外观");
            RegisterProperty(typeof(TodayWeather), "WindowStyleName", typeof(SelectStylesEditor), result, "样式名称", "外观");
            // 组别：渲染
            RegisterProperty(typeof(TodayWeather), "RenderMethod", null, result, "加载方式", "渲染");
            RegisterProperty(typeof(TodayWeather), "RenderOrder", null, result, "渲染优先级", "渲染");
